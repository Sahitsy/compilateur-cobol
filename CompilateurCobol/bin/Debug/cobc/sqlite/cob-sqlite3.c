...
static int (*func)(char *errnooo, const char *errmsg); 
sqlite3 *dbx;
...
/*　　　　　　　　　errnooo, errmsg をパラメタとして指定されたプログラムを呼ぶ。   */
    ...
    switch(errout){
        ...
        case 2:
             break;
        case 3:
             sprintf(errnooo, "%02d", rc);
             func(errnooo, err_msg);
    }
    return;
}
...
// Ajout SRA -DEB
// BIND DOUBLE
int SQLite3_bind_double(sqlite3_stmt **stmt, int *iCol, double *var)
{
   rc = sqlite3_bind_double(*stmt, *iCol, *var);
   if( rc ){
       sprintf(err_msg, "sqlite3_bind_double:%s", sqlite3_errmsg(dbx));
   }
   err_exit(rc);
   return sqlite3_errcode(dbx);
}
// BIND VALUE
/*
int SQLite3_bind_value(sqlite3_stmt **stmt, int *iCol, sqlite3_value **valeur)
{
   rc = sqlite3_bind_value(*stmt, *iCol, *valeur);
   if( rc ){
       sprintf(err_msg, "sqlite3_bind_value:%s", sqlite3_errmsg(dbx));
   }
   err_exit(rc);
   return sqlite3_errcode(dbx);
}
